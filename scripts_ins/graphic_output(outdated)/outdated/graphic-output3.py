...
#define a superlist with innerlists, each of them containing all the info from each contig 
superlist = list()
	...
	for t in tag_list:
		...
		for i, line in enumerate(lines):
			if not line.startswith('@'): 
				...
				if sp[5].strip() == 'TOTAL' and contig == c[0].strip():  #<---------------------------------------------------
					tag2 = contig + '-' + insertion
					if tag2.strip() == t.strip() :
						n_reads = n_reads + 1
					...
					elif tag2.strip() != t.strip() :
						if aprox_position != 0 and aprox_position not in position_list: 
							position_list.append(aprox_position)
							print str(aprox_position) + 'i1'

							
		if aprox_position != 0 and aprox_position not in position_list: 
			position_list.append(aprox_position)
			contig = None
			print str(aprox_position) + 'i2'
