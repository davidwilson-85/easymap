...
parser.add_argument('-read_s_par', action="store",default = 'n/p', dest = 'read_s_par')
parser.add_argument('-read_f_par', action="store",default = 'n/p', dest = 'read_f_par')
parser.add_argument('-read_r_par', action="store",default = 'n/p', dest = 'read_r_par')
parser.add_argument('-cross_type', action="store",default = 'n/p', dest = 'cross_type', choices=set(('oc','bc')))
parser.add_argument('-is_ref_strain', action="store",default = 'n/p', dest = 'is_ref_strain', choices=set(('ref','noref'))) #We could give the chance not to be provided in F2 sequenced cases
parser.add_argument('-parental_used_as_control', action="store",default = 'n/p', dest = 'parental_used_as_control', choices=set(('mut','nomut'))) #We could give the possibility not to provide (usage of 2 F2 or reference as parental)
...
read_s_par = args.read_s_par
read_f_par = args.read_f_par
read_r_par = args.read_r_par
...
if data_source == "exp":
	...
	#For snp mode, parental reads should be provided for the further procesing. ---> it is possible we will not require parental genotyping, in that case here, give the reference as parenta
	if workflow == "snp":
		if lib_type == "pe":
...
master_program_input = project_name + " " + workflow + " " + data_source + " "+ lib_type + " " + ref_seqs + " " + ins_seq + " " + read_s + " " + read_f + " " + read_r + " " + gff_file + " " + ann_file + " " + sim_mut + " " + sim_recsel + " " + sim_seq + " " + read_s_par + " " + read_f_par + " " + read_r_par + " " + is_ref_strain+ " "+ cross_type+ " "+ parental_used_as_control 
