		...
		try: 
			...
			if len(values) == 4:
				...
				for ch in chromo:
					...
					for v in c:
						v = v.split(",")
						if len(v) != 2 or not v[0] or not v[1] : error = 1, problems.append("One or more than one of the values you supplied as recombination frequences for sim_recsel input in simulator does not contain the two required values 2,50 being 2 the number of events of recombination and 50 the probability of happening.") 
						if not v[1]: error =1
						if not v[0]: error =1 
				val1 = values[1].split(",")
				int(val1[0])  
